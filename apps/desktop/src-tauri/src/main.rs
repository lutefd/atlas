...
struct TimelineEvent {
    ...
    source_parser_output_id: Option<String>,
    created_at: String,
}
...
struct Snapshot {
    ...
    entities: Vec<Entity>,
    tags: Vec<Tag>,
    parser_outputs: Vec<ParserOutput>,
    jobs: Vec<Job>,
}

#[derive(Deserialize)]
struct CreateEvidenceInput {
    incident_id: String,
    kind: String,
    source: String,
    content_text: Option<String>,
...
struct DerivedTimelineInput {
    ...
    source_parser_output_id: String,
}

#[derive(Deserialize)]
struct DerivedEntityInput {
    id: String,
    incident_id: String,
    entity_type: String,
    name: String,
...
fn update_job(state: tauri::State<AppState>, input: UpdateJobInput) -> Result<(), String> {
    ...
    open(&state.db_path)?
        .execute(
            ...
            params![input.status, input.error_text, now(), input.id],
        )
        .map_err(|error| error.to_string())?;
    Ok(())
}
...
#[tauri::command]
fn create_manual_timeline_event(
...
#[tauri::command]
fn update_manual_timeline_event(
    state: tauri::State<AppState>,
...
#[tauri::command]
fn delete_manual_timeline_event(
    state: tauri::State<AppState>,
    event_id: String,
) -> Result<(), String> {
    let _guard = state.lock.lock().map_err(|error| error.to_string())?;
...
fn add_tag(state: tauri::State<AppState>, incident_id: String, name: String) -> Result<(), String> {
    ...
    open(&state.db_path)?
        .execute(
            ...
            params![id(), incident_id, name.trim(), now()],
        )
        .map_err(|error| error.to_string())?;
    Ok(())
}
...
#[tauri::command]
fn select_export_directory() -> Option<String> {
    rfd::FileDialog::new()
...
#[tauri::command]
fn select_import_directory() -> Option<String> {
    rfd::FileDialog::new()
...
#[tauri::command]
fn export_incident_to_directory(
    state: tauri::State<AppState>,
    incident_id: String,
    destination: String,
) -> Result<String, String> {
...
fn import_incident(state: tauri::State<AppState>, export_path: String) -> Result<(), String> {
    ...
    tx.execute(
        ...
        params![
            incident_id,
            json_string(&incident, "title")?,
            json_string(&incident, "created_at")?,
            json_string(&incident, "updated_at")?
        ],
    )
    .map_err(|error| error.to_string())?;

    for row in read_json(&export_dir.join("evidence.json"))?
        .as_array()
        .cloned()
        .unwrap_or_default()
    {
        tx.execute(
            ...
            params![
                json_string(&row, "id")?,
                json_string(&row, "incident_id")?,
                json_string(&row, "kind")?,
                json_string(&row, "source")?,
                json_optional_string(&row, "content_text"),
                json_string(&row, "content_hash")?,
                json_string(&row, "created_at")?,
                json_string(&row, "metadata_json")?,
                json_optional_string(&row, "attachment_id")
            ],
        )
        .map_err(|error| error.to_string())?;
    }
    for row in read_json(&export_dir.join("parser_outputs.json"))?
        .as_array()
        .cloned()
        .unwrap_or_default()
    {
        tx.execute(
            ...
            params![
                json_string(&row, "id")?,
                json_string(&row, "evidence_id")?,
                json_string(&row, "parser_name")?,
                json_string(&row, "parser_version")?,
                json_string(&row, "output_json")?,
                json_string(&row, "created_at")?
            ],
        )
        .map_err(|error| error.to_string())?;
    }
    for row in read_json(&export_dir.join("timeline.json"))?
        .as_array()
        .cloned()
        .unwrap_or_default()
    {
        tx.execute(
            ...
            params![
                ...
                row.get("confidence")
                    ...
                    .unwrap_or(1.0),
                json_optional_string(&row, "source_evidence_id"),
                json_optional_string(&row, "source_parser_output_id"),
                json_string(&row, "created_at")?
            ],
        )
        .map_err(|error| error.to_string())?;
    }
    for row in read_json(&export_dir.join("entities.json"))?
        .as_array()
        .cloned()
        .unwrap_or_default()
    {
    ...
    for row in read_json(&export_dir.join("tags.json"))?
        .as_array()
        .cloned()
        .unwrap_or_default()
    {
    ...
    for row in read_json(&export_dir.join("attachments.json"))?
        .as_array()
        .cloned()
        .unwrap_or_default()
    {
        ...
        tx.execute(
            ...
            params![
                ...
                json_optional_string(&row, "mime_type"),
                row.get("size_bytes")
...
fn main() {
    tauri::Builder::default()
        ...
        .setup(|app| {
            ...
            migrate(&open(&db_path).map_err(|error| Box::<dyn std::error::Error>::from(error))?)
        ...
        .invoke_handler(tauri::generate_handler![
            ...
            update_manual_timeline_event,
            delete_manual_timeline_event,
            add_tag,
            delete_tag,
            clear_evidence_parsers,
            delete_evidence,
            delete_incident,
            load_snapshot,
            search,
