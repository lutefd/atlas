...
export type IncidentDto = { id: string; title: string; created_at: string; updated_at: string };
export type EvidenceDto = { id: string; incident_id: string; kind: Evidence["kind"]; source: string; content_text: string | null; content_hash: string; created_at: string; metadata_json: string; attachment_id: string | null };
export type TimelineEventDto = { id: string; incident_id: string; timestamp: string; title: string; description: string; confidence: number; source_evidence_id: string | null; source_parser_output_id: string | null; created_at: string };
...
function parseOutput(value: string): ParsedOutput {
  try {
    ...
    return { entities: [], timestamps: [], events: [], metrics: [], references: [] };
  }
}
...
export function toEvidence(row: EvidenceDto): Evidence {
  return { id: row.id, incidentId: row.incident_id, kind: row.kind, source: row.source, contentText: row.content_text, contentHash: row.content_hash, createdAt: row.created_at, metadata: parseMetadata(row.metadata_json), attachmentId: row.attachment_id };
}

export function toTimelineEvent(row: TimelineEventDto): TimelineEvent {
  return { id: row.id, incidentId: row.incident_id, timestamp: row.timestamp, title: row.title, description: row.description, confidence: row.confidence, sourceEvidenceId: row.source_evidence_id, sourceParserOutputId: row.source_parser_output_id, createdAt: row.created_at };
}

export function toEntity(row: EntityDto): Entity {
  return { id: row.id, incidentId: row.incident_id, type: row.entity_type, name: row.name, confidence: row.confidence, sourceEvidenceId: row.source_evidence_id, sourceParserOutputId: row.source_parser_output_id, createdAt: row.created_at };
}
...
export async function createManualTimelineEvent(input: { incidentId: string; timestamp: string; title: string; description: string; sourceEvidenceId?: string | null }): Promise<TimelineEvent> {
  return toTimelineEvent(await invoke<TimelineEventDto>("create_manual_timeline_event", { input: { incident_id: input.incidentId, timestamp: input.timestamp, title: input.title, description: input.description, source_evidence_id: input.sourceEvidenceId ?? null } }));
}

export async function updateManualTimelineEvent(input: { id: string; timestamp: string; title: string; description: string; sourceEvidenceId?: string | null }) {
  await invoke("update_manual_timeline_event", { input: { id: input.id, timestamp: input.timestamp, title: input.title, description: input.description, source_evidence_id: input.sourceEvidenceId ?? null } });
}
...
export async function renameIncident(incidentId: string, title: string): Promise<Incident> {
  return toIncident(await invoke<IncidentDto>("rename_incident", { incidentId, title }));
}

export async function addEvidence(input: { incidentId: string; kind: Evidence["kind"]; source: string; text?: string; metadata?: Record<string, unknown>; attachmentName?: string; attachmentMimeType?: string | null; attachmentBase64?: string }): Promise<Evidence> {
  return toEvidence(await invoke<EvidenceDto>("add_evidence", { input: { incident_id: input.incidentId, kind: input.kind, source: input.source, content_text: input.text ?? null, metadata_json: JSON.stringify(input.metadata ?? {}), attachment_name: input.attachmentName, attachment_mime_type: input.attachmentMimeType ?? null, attachment_base64: input.attachmentBase64 } }));
}
