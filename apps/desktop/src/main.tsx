...
import { addEvidence, addTag, clearEvidenceParsers, createIncident, createJob, createManualTimelineEvent, deleteEvidence, deleteIncident, deleteManualTimelineEvent, deleteTag, exportIncident, importIncident, loadAttachment, loadSnapshot, openAttachment, renameIncident, revealAttachment, saveParserOutput, search, updateJob, updateManualTimelineEvent, type AttachmentData, type Job, type SearchResult, type Snapshot } from "./api";
import "./styles.css";
...
function Workspace() {
  ...
  async function exportActiveIncident() {
    ...
    try {
      ...
      setIncidentStatus(caught instanceof Error ? caught.message : String(caught));
    }
  }
  ...
  return <main className="shell">
    <aside className="sidebar">
      ...
      <div className="import-box"><input value={importPath} onChange={(event) => setImportPath(event.target.value)} placeholder="Import incident folder path" /><button onClick={() => void importIncidentFromPath()} disabled={!importPath.trim()}>Import</button></div>
...
function RightPanel({ incidentId, timeline, entities, tags, evidence, onSelectEvidence }: { incidentId: string; timeline: Snapshot["timelineEvents"]; entities: Snapshot["entities"]; tags: Snapshot["tags"]; evidence: Evidence[]; onSelectEvidence: (id: string) => void }) {
  const queryClient = useQueryClient();
  const [tag, setTag] = useState("");
  const [query, setQuery] = useState("");
  const [searchFilter, setSearchFilter] = useState("all");
  const [manualEvent, setManualEvent] = useState({ timestamp: new Date().toISOString().slice(0, 16), title: "", description: "", sourceEvidenceId: "" });
  ...
  const filteredResults = searchFilter === "all" ? results : results.filter((result) => result.kind === searchFilter);
  ...
  const saveManualEventMutation = useMutation({ mutationFn: async () => {
    const input = { timestamp: new Date(manualEvent.timestamp).toISOString(), title: manualEvent.title, description: manualEvent.description, sourceEvidenceId: manualEvent.sourceEvidenceId || null };
    if (editingEventId) await updateManualTimelineEvent({ id: editingEventId, ...input });
    else await createManualTimelineEvent({ incidentId, ...input });
  }, onSuccess: () => { closeEventModal(); queryClient.invalidateQueries({ queryKey: ["snapshot"] }); } });
  const deleteManualEventMutation = useMutation({ mutationFn: deleteManualTimelineEvent, onSuccess: () => { queryClient.invalidateQueries({ queryKey: ["snapshot"] }); } });
  const groupedEntities = useMemo(() => entities.slice(0, 20), [entities]);
  function openNewEventModal() {
    setEditingEventId(null);
    setManualEvent({ timestamp: new Date().toISOString().slice(0, 16), title: "", description: "", sourceEvidenceId: "" });
    setIsEventModalOpen(true);
  }
  function editManualEvent(event: Snapshot["timelineEvents"][number]) {
    setEditingEventId(event.id);
    setManualEvent({ timestamp: new Date(event.timestamp).toISOString().slice(0, 16), title: event.title, description: event.description, sourceEvidenceId: event.sourceEvidenceId ?? "" });
    setIsEventModalOpen(true);
  }
  ...
  function openSearchResult(result: SearchResult) {
    if (result.kind === "evidence" || result.kind === "attachment") onSelectEvidence(result.refId);
  ...
  return <aside className="right">
    <section className="panel"><div className="panel-heading"><h2><Search size={16} /> Search</h2>{query ? <button onClick={() => setQuery("")}>Clear</button> : null}</div><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search evidence, events, entities, files" /><div className="search-filters">{["all", "evidence", "timeline", "entity", "attachment"].map((filter) => <button className={searchFilter === filter ? "active" : ""} key={filter} onClick={() => setSearchFilter(filter)}>{filter}</button>)}</div>{query.trim() && !safeQuery ? <p className="muted">Use letters or numbers to search.</p> : null}{searchError ? <p className="error">Search failed: {String(searchError)}</p> : null}{safeQuery && results.length === 0 && !searchError ? <p className="muted">No results.</p> : null}{safeQuery && results.length > 0 && filteredResults.length === 0 ? <p className="muted">No {searchFilter} results.</p> : null}{filteredResults.map((result) => <button className="result" key={`${result.kind}-${result.refId}`} onClick={() => openSearchResult(result)}><strong>{result.kind}: {result.title}</strong><span dangerouslySetInnerHTML={{ __html: result.snippet }} /></button>)}</section>
    <section className="panel timeline-panel"><div className="panel-heading"><h2>Timeline</h2><button onClick={openNewEventModal}>Add event</button></div><div className="timeline-list">{timeline.length ? timeline.map((event) => <div className={event.sourceParserOutputId ? "event derived" : "event manual"} key={event.id}><button onClick={() => event.sourceEvidenceId && onSelectEvidence(event.sourceEvidenceId)}><time>{new Date(event.timestamp).toLocaleString()}</time><strong>{event.title}</strong><span>{event.sourceParserOutputId ? `${Math.round(event.confidence * 100)}% · derived · source ${event.sourceEvidenceId?.slice(0, 8)}` : `manual${event.sourceEvidenceId ? ` · source ${event.sourceEvidenceId.slice(0, 8)}` : ""}`}</span></button>{!event.sourceParserOutputId ? <div className="event-actions"><button onClick={() => editManualEvent(event)}>Edit</button><button onClick={() => deleteManualEventMutation.mutate(event.id)}>Delete</button></div> : null}</div>) : <p className="muted">No timeline events yet. Atlas only adds derived events when parsers find incident signals such as timestamps, deploys, errors, timeouts, or 5xx statuses.</p>}</div>{isEventModalOpen ? <div className="modal-backdrop" onClick={closeEventModal}><div className="modal" onClick={(event) => event.stopPropagation()}><div className="modal-header"><h2>{editingEventId ? "Edit event" : "Add event"}</h2><button className="icon-button" onClick={closeEventModal}><X size={14} /></button></div><div className="manual-event-form"><input type="datetime-local" value={manualEvent.timestamp} onChange={(event) => setManualEvent((value) => ({ ...value, timestamp: event.target.value }))} /><input value={manualEvent.title} onChange={(event) => setManualEvent((value) => ({ ...value, title: event.target.value }))} placeholder="Manual event title" /><textarea value={manualEvent.description} onChange={(event) => setManualEvent((value) => ({ ...value, description: event.target.value }))} placeholder="What happened?" /><select value={manualEvent.sourceEvidenceId} onChange={(event) => setManualEvent((value) => ({ ...value, sourceEvidenceId: event.target.value }))}><option value="">No source evidence</option>{evidence.map((item) => <option value={item.id} key={item.id}>{item.source} · {item.kind}</option>)}</select><div className="manual-event-actions"><button onClick={() => saveManualEventMutation.mutate()} disabled={!manualEvent.title.trim() || !manualEvent.timestamp}>{editingEventId ? "Update event" : "Add event"}</button><button onClick={closeEventModal}>Cancel</button></div></div></div></div> : null}</section>
