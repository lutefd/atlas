...
function Workspace() {
  ...
  const deleteIncident = useMutation({ mutationFn: (incidentId: string) => invoke("delete_incident", { incidentId }), onSuccess: () => { useUi.setState({ selectedIncidentId: null }); queryClient.invalidateQueries({ queryKey: ["snapshot"] }); } });
  function confirmDeleteIncident(incident: IncidentRow) {
    if (confirm(`Delete ${incident.title}? This removes its evidence, derived timeline, entities, tags, and local attachments.`)) {
...
function EvidenceInbox({ incidentId }: { incidentId: string }) {
  ...
  return <div className="inbox" onPasteCapture={handlePaste} onDragOver={(event) => event.preventDefault()} onDrop={(event) => { event.preventDefault(); Array.from(event.dataTransfer.files).forEach((file) => void addFile(file, "drop")); }}>
    ...
    <CodeMirror value={text} minHeight="160px" placeholder="Paste logs, Slack snippets, commands, notes, or incident observations..." onChange={setText} />
...
function EvidenceStream({ evidence }: { evidence: Evidence[] }) {
  return <section><h2>Evidence</h2><div className="stream">{evidence.map((item) => <EvidenceCard item={item} key={item.id} />)}</div></section>;
}
...
function EvidenceCard({ item }: { item: Evidence }) {
  ...
  const deleteEvidence = useMutation({ mutationFn: () => invoke("delete_evidence", { evidenceId: item.id }), onSuccess: () => { queryClient.invalidateQueries({ queryKey: ["snapshot"] }); queryClient.removeQueries({ queryKey: ["attachment", item.id] }); } });
